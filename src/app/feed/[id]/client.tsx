...
import AddTagImage from "@/public/assets/project/tag-add-plus.svg"
...
export default function ProjectPageClient({data, tasks, posts}: {data: ProjectFull, tasks: TaskShortDTO[], posts: PostShortDTO[]}) {
  ...
  const handleAddTag = () => {
    ...
    const handleSubmit = async (e: React.SubmitEvent) => {
      ...
      try {
        ...
        const response = await updateProject(projectUpdateRequestData);
  ...
  return (
    <div className={`pagecontainer ${styles.container}`}>
      ...
      <div className={styles.projects}>
        ...
        {isAdmin && <>
            { posts.length > 0 && <>
            {posts.map((value, index) => <PostCardAdmin {...value} key={index} deleteAction={deletePost} />)}
          </>}
            { tasks.length > 0 && <>
