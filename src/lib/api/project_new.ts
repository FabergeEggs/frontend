...
export const createProject = async (request: ProjectCreateDTO) => {
  const { data } = await api.post(ApiRoutes.PROJECT, request);
  return data;
};

export const updateProject = async (project_id: string, request: ProjectCreateDTO) => {
  const { data } = await api.put(`${ApiRoutes.PROJECT}/${project_id}`, request);
  return data;
};

export const getProjectInfo = async (id: string): Promise<ProjectInfoDTO> => {
  const { data } = await api.get(`${ApiRoutes.PROJECT}/${id}/info`);
  return data;
};
...
export const getPublications = async (project_id: string): Promise<PublicationsResponse> => {
  const { data } = await api.get(`${ApiRoutes.PROJECT}/${project_id}/publications`);
  return data;
}
...
export const getUserMemberships = async (profile_id: string): Promise<MembershipsDTO> => {
  const { data } = await api.get(`${ApiRoutes.PROJECT}/profile/${profile_id}`);
  return data;
}

export const addMember = async (project_id: string, request: AddMemberDTO) => {
  const { data } = await api.post(`${ApiRoutes.PROJECT}/${project_id}/member`, request);
  return data;
}

export const removeMember = async (project_id: string, user_id: string) => {
  const { data } = await api.delete(`${ApiRoutes.PROJECT}/${project_id}/member/${user_id}`);
  return data;
}
...
export const createTask = async (project_id: string, request: TaskCreateDTO) => {
  const { data } = await api.post(`${ApiRoutes.PROJECT}/${project_id}/task`, request);
  return data;
};

export const getTask = async (project_id: string, task_id: string): Promise<Task> => {
  const { data } = await api.get(`${ApiRoutes.PROJECT}/${project_id}/task/${task_id}`);
  return data;
};

export const updateTask = async (project_id: string, task_id: string, request: TaskUpdateDTO) => {
  const { data } = await api.put(`${ApiRoutes.PROJECT}/${project_id}/task/${task_id}`, request);
  return data;
};
...
export const createPost = async (project_id: string, request: PostCreateDTO) => {
  const { data } = await api.post(`${ApiRoutes.PROJECT}/${project_id}/post`, request);
  return data;
};

export const getPost = async (project_id: string, post_id: string): Promise<Post> => {
  const { data } = await api.get(`${ApiRoutes.PROJECT}/${project_id}/post/${post_id}`);
  return data;
};

export const updatePost = async (project_id: string, post_id: string, request: PostCreateDTO) => {
  const { data } = await api.put(`${ApiRoutes.PROJECT}/${project_id}/post/${post_id}`, request);
  return data;
};

export const deletePost = async (project_id: string, post_id: string) => {
  const { data } = await api.delete(`${ApiRoutes.PROJECT}/${project_id}/post/${post_id}`);
  return data;
};